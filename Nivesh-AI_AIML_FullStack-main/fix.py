...
file_path = '/Users/vishalgautam/Downloads/wealthpulse-main/app/components/CallToAction.jsx'
with open(file_path, 'r') as f:
    lines = f.readlines()

content_before = ''.join(lines[:543])

# Construct the new return statement exactly
new_return = """  return (
    <motion.section
      ref={(el) => {
      ...
      className="relative min-h-[100vh] flex flex-col w-full overflow-hidden bg-slate-900 pt-[72px] lg:pt-20 group"
    >
      ...
      <div className="relative z-10 flex-1 w-full px-6 md:px-12 py-12 flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-20 max-w-[1700px] mx-auto min-h-[50vh]">
        ...
        <div className="w-full lg:w-[50%] flex justify-center items-center relative group/video mt-8 lg:mt-0">
           <div className="absolute -inset-2 bg-gradient-to-r from-blue-500 to-sky-500 rounded-[2.5rem] blur-xl opacity-20 group-hover/video:opacity-40 transition duration-500"></div>
           <video
             ...
             playsInline
             controls={false}
             className="relative w-full h-auto max-h-[60vh] rounded-[2.5rem] shadow-2xl object-cover border border-white/20"
           />
        </div>
...
with open(file_path, 'w') as f:
    f.write(content_before + new_return)
